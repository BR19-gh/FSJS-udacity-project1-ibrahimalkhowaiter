...
const FULL_IMAGES_PATH = './images/full';
const PROCESSED_IMAGES_PATH = './images/processed';
...
export default {
    ...
    async checkIfOriginalImageExists(): Promise<boolean> {
        try {
            const fileData = await fsAsync.open(this.getStockImagePath(), 'r');
    ...
    async checkIfTransformedImageExists(): Promise<boolean> {
        try {
            const fileData = await fsAsync.open(this.getImagePath(), 'r');
            await fileData.close();
            return true;
        } catch (e) {
            return false;
     ...
     */
    init: async function Constructor(imageId: string, width = 0, height = 0): Promise<SharpImage> {
        if (!imageId) {
            throw new Error(
                JSON.stringify({
                    status: 400,
        ...
        if (this.isOriginal) {
            ...
            const originalImageExists = await this.checkIfOriginalImageExists();
            ...
            const transformedImageExists = await this.checkIfTransformedImageExists();
            ...
            if (!transformedImageExists) {
                ...
                if (originalImageExists) {
                    ...
                    await fsAsync.mkdir(`${PROCESSED_IMAGES_PATH}/${this.width}x${this.height}`, { recursive: true });

                    //3.5 Create the file binary
                    const fileData = await fsAsync.open(this.getImagePath(), 'w');

                    //3.6 Write image data to file
                    await fileData.write(await sharp(this.getStockImagePath()).resize(width, height).toBuffer());
